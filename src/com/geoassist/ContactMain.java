...
public class ContactMain  extends BaseListActivity implements OnClickListener {
	static final int ADD_CONTACT_ACTIVITY = 100;
	private ContactListAdaptor expListAdapter = null;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (proj.sites.size( )>= 0) {
			...
			for (int i =0; i< site.contacts.size();i++) {
				groupItem.add("Contact "+site.contacts.get(i).contactType);
				ArrayList<String> child = new ArrayList<String>();
				child.add("Dummy");
				childItem.add(child);
			}
		}
		expListAdapter = new ContactListAdaptor(groupItem, childItem);
	...
	public void onClick(View v) {
		switch (v.getId()) {
		case R.id.add:
			invokeActivity(AddContactActivity.class, ADD_CONTACT_ACTIVITY);
			break;
		default:
			break;
		}
	...
	private void refreshContactList() {
		...
		if (proj.sites.size( )>= 0) {
			Site site = proj.sites.get(proj.sites.size()-1);
			for (int i =0; i< site.contacts.size();i++) {
				groupItem.add("Contact "+site.contacts.get(i).contact1Name);
				ArrayList<String> child = new ArrayList<String>();
				child.add("Dummy");
				childItem.add(child);
			}
